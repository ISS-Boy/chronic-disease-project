...
import org.smartloli.kafka.eagle.grafana.D3GaugePanels.Targets;
...
public class WriteD3Panels {
	...
	public List<Targets> makeTargetsList(List<PARMOfTarget> parmOfTargetslist){
		if(null!=parmOfTargetslist&&parmOfTargetslist.size()<2) {
			...
			target.setTags(parmOfTargetslist.get(0).getTags());
			...
			for(int i=0;i<parmOfTargetslist.size();i++) {
				...
				newtarget.setTags(parmOfTargetslist.get(i).getTags());
